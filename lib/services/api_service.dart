...
class ApiService {
  ...
  Future<User> login(String email, String password) async {
    ...
    if (response.statusCode == 200) {
      ...
      final routes = (userData['route'] as List)
          .map((r) => RouteData(
                routeId: r['route_id'],
                vehicle: r['vehicle'],
                warehouse: r['warehouse'],
                warehouseName: r['warehouse_name'], stops: [],
              ))
          .toList();
      return User(
        token: token,
...
print('Response status: $response');
  ...
  Future<List<Item>> listItems(String routeId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/route_plan.apis.sales.list_warehouse_items?route_id=$routeId'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  Future<Map<String, dynamic>> raiseInvoice(String routeId, String stopId, String day, List<Item> items) async {
    final response = await http.post(
      ...
      body: jsonEncode({
        ...
        'stop_id': stopId,
        'day': day,
        'items': items.map((item) => {'item_code': item.itemCode, 'qty': item.quantity}).toList(),
      }),
    );
    if (response.statusCode == 200 || response.statusCode == 201) {
      final data = jsonDecode(response.body);
  ...
  Future<void> createPayment(String invoiceId, double amount, String mode) async {
    final response = await http.post(
